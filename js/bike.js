// js/bike.js — Micheluccio (calvo, giubbotto Ducati) e la sua sportiva rossa.
// Modello low-poly costruito da primitive three.js. Avanti = +z, su = +y.
import * as THREE from '../vendor/three.module.js';

const DUCATI_RED = 0xcc0000;

function mats() {
  const std = (color, roughness, metalness = 0) =>
    new THREE.MeshStandardMaterial({ color, roughness, metalness });
  return {
    red: std(DUCATI_RED, 0.3, 0.25),
...
export function buildBike() {
  ...
  steer.add(box(0.07, 0.04, 0.02, M.black, -0.27, 0.45, -0.06));
  // cupolino + faro + parafango
  steer.add(box(0.28, 0.26, 0.42, M.red, 0, 0.14, 0.08, -0.22));
  const headlight = box(0.16, 0.08, 0.03, M.white, 0, 0.1, 0.3, -0.22);
  headlight.material = new THREE.MeshStandardMaterial({ color: 0xfffbe8, emissive: 0x777055, roughness: 0.2 });
  steer.add(headlight);
  steer.add(box(0.14, 0.03, 0.34, M.red, 0, 0.14, 0.16, 0.12));
  // scritta DUCATI sulle fiancate del cupolino
  const sideR = textPlane('DUCATI', 0.3, 0.08, '#ffffff');
  if (sideR) { sideR.position.set(0.145, 0.16, 0.08); sideR.rotation.y = Math.PI / 2; steer.add(sideR); }
  const sideL = textPlane('DUCATI', 0.3, 0.08, '#ffffff');
  ...
  const tank = new THREE.Mesh(new THREE.SphereGeometry(1, 16, 12), M.red);
  tank.scale.set(0.21, 0.17, 0.34);
  tank.position.set(0, 0.74, 0.12);
  lean.add(tank);
  lean.add(box(0.2, 0.05, 0.45, M.black, 0, 0.7, -0.33));             // sella
  lean.add(box(0.17, 0.09, 0.3, M.red, 0, 0.73, -0.58, 0.14));        // codino
  ...
  // ---- MICHELUCCIO: calvo, giubbotto Ducati rosso, jeans
  const rider = new THREE.Group();
  lean.add(rider);
  rider.add(box(0.3, 0.18, 0.22, M.jeans, 0, 0.82, -0.28));           // bacino
  rider.add(box(0.36, 0.48, 0.24, M.red, 0, 1.12, -0.16, 0.25));      // busto (giubbotto)
  // braccia: spalla -> gomito -> manopola
  for (const s of [1, -1]) {
    rider.add(limb(0.21 * s, 1.28, -0.12, 0.24 * s, 1.05, 0.12, 0.055, M.red));
    rider.add(limb(0.24 * s, 1.05, 0.12, 0.19 * s, 0.9, 0.55, 0.048, M.red));
    ...
    rider.add(limb(0.13 * s, 0.8, -0.26, 0.22 * s, 0.55, 0.05, 0.075, M.jeans));
    rider.add(limb(0.22 * s, 0.55, 0.05, 0.16 * s, 0.34, -0.02, 0.06, M.jeans));
    rider.add(box(0.09, 0.08, 0.22, M.black, 0.16 * s, 0.3, 0.0));
  }
  // collo + testa calva + occhiali da sole
  ...
  rider.add(head);
  rider.add(box(0.17, 0.05, 0.03, M.black, 0, 1.5, 0.045));
  // scritta DUCATI sulla schiena
  const back = textPlane('DUCATI', 0.32, 0.09, '#ffffff');
  if (back) {
